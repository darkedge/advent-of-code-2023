...
mul :: proc (leftBegin, leftEnd, rightBegin, rightEnd: int) -> int {
  left := strconv.parse_int(input[leftBegin:leftEnd]) or_else 0
  right := strconv.parse_int(input[rightBegin:rightEnd]) or_else 0
  product := left * right
  // fmt.printfln("mul({},{}) = {}", left, right, product)
  return product
}
